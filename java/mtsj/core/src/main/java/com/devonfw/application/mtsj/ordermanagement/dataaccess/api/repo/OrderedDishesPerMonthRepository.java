...
public interface OrderedDishesPerMonthRepository extends DefaultRepository<OrderedDishesPerMonthEntity> {
   ...
   *
   * @return the {@link Page} with the matching {@link OrderedDishesPerMonthEntity} objects.
   */
  default Page<OrderedDishesPerMonthEntity> findOrderedDishesPerMonth(OrderedDishesSearchCriteriaTo criteria) {
    ...
    // le,ge,lt,gt,eq,ne
    Timestamp startBookingdate = new Timestamp(criteria.getStartBookingdate().getYear(),
        criteria.getStartBookingdate().getMonth(), criteria.getStartBookingdate().getDate() - 1, 23, 59, 59, 999);
    if (startBookingdate != null) {
      query.where(Alias.$(orderedDish.getBookingdate()).after(startBookingdate));
    }
    Timestamp endBookingdate = new Timestamp(criteria.getEndBookingdate().getYear(),
        criteria.getEndBookingdate().getMonth(), criteria.getEndBookingdate().getDate(), 23, 59, 59, 999);
