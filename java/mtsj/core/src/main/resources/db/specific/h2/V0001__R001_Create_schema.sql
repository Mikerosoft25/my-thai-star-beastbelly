...
CREATE TABLE PasswordResetToken (
  ...
  CONSTRAINT FK_PasswordResetToken_idUser FOREIGN KEY(idUser) REFERENCES USER (id) ON DELETE CASCADE NOCHECK
);
...
CREATE TABLE Booking (
  ...
  CONSTRAINT FK_Booking_idUser FOREIGN KEY(idUser) REFERENCES User(id) ON DELETE SET NULL NOCHECK,
...
CREATE TABLE InvitedGuest (
  ...
  CONSTRAINT FK_InvitedGuest_idBooking FOREIGN KEY(idBooking) REFERENCES Booking(id) ON DELETE CASCADE NOCHECK
);

-- *** OrderPayStatus ***
CREATE TABLE OrderPayStatus (
  id BIGINT NOT NULL AUTO_INCREMENT,
  modificationCounter INTEGER NOT NULL,
  payStatus VARCHAR (255),
  CONSTRAINT PK_OrderPayStatus PRIMARY KEY(id)
);
...
CREATE TABLE Orders (
  ...
  modificationCounter INTEGER NOT NULL,
  -- idBooking BIGINT NOT NULL,
  idBooking BIGINT, -- NULL now allowed, indicates that its a delivery order
  idInvitedGuest BIGINT,
  idHost BIGINT,
  idOrderStatus BIGINT NOT NULL,
  idOrderPayStatus BIGINT NOT NULL,
  CONSTRAINT PK_Order PRIMARY KEY(id),
  CONSTRAINT FK_Order_idBooking FOREIGN KEY(idBooking) REFERENCES Booking(id) ON DELETE CASCADE NOCHECK,
  CONSTRAINT FK_Order_idInvitedGuest FOREIGN KEY(idInvitedGuest) REFERENCES InvitedGuest(id) NOCHECK,
  CONSTRAINT FK_Order_idOrderStatus FOREIGN KEY(idOrderStatus) REFERENCES OrderStatus(id) NOCHECK,
  CONSTRAINT FK_Order_idOrderPayStatus FOREIGN KEY(idOrderPayStatus) REFERENCES OrderPayStatus(id) NOCHECK
);
...
CREATE TABLE OrderLine (
  ...
  CONSTRAINT FK_OrderLine_idOrder FOREIGN KEY(idOrder) REFERENCES Orders(id) ON DELETE CASCADE NOCHECK
);
...
CREATE TABLE OrderDishExtraIngredient (
  ...
  CONSTRAINT FK_OrderDishExtraIngredient_idOrderLine FOREIGN KEY(idOrderLine) REFERENCES OrderLine(id) ON DELETE CASCADE NOCHECK,
