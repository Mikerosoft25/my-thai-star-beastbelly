...
export class AuthEffects {
  private readonly restPathRoot$: Observable<string> =
    this.config.getRestPathRoot();
  private readonly verifyRestPath: string = 'verify';
  private restServiceRoot$: Observable<string> =
    ...
    this.actions$.pipe(
      ...
      switchMap((user: any) => {
        return this.userService.login(user.username, user.password).pipe(
          ...
          catchError((error) =>
            of(authActions.openDialog(), authActions.loginFail({ error })),
          ),
        );
      }),
    ),
  ...
  verifyTwoFactor$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((user) => {
        const dialogRef: MatDialogRef<TwoFactorDialogComponent> =
  ...
  loginFail$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        tap(() => {
          this.snackBar.openSnack(
            this.translocoService.translate('alerts.authAlerts.loginFail'),
            4000,
            'red',
          );
        }),
